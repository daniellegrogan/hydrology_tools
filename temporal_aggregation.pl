...
my ($help,$verbose,$remove,$rm_rec,$agg_type,$sclOffs,$patch,$trim,$push,$mdt,$type,$s_date,$e_date,$MT_file) =
   (  0,     0,       0,      0,       0,      undef,  'N/A',   0,   0,   '',   '',    '',     ''  ,   ''   );
# print "$sclOffs[0] $sclOffs[1] $agg_type\n";
						# Get command line options
usage() if !GetOptions('h'=>\$help, 'v'=>\$verbose, 'rm'=>\$remove, 'rr=i'=>\$rm_rec, 'a=i'=>\$agg_type, 'so=i{2}'=>\@$sclOffs, 'trim'=>\$trim, 'pd'=>\$push, 'mdt=s'=>\$mdt, 'ot=s'=>\$type, 'sd=s'=>\$s_date, 'ed=s'=>\$e_date, 'mt=s'=>\$MT_file, 'p=f'=>\$patch) or $help;
...
sub crs_to_Obj
{
  ...
  my $crs	= new Geo::OSR::SpatialReference(EPSG=>$epsgID);

  if    ($epsg	=~ s/epsg:(\d+)/$1/i) {		### Import EPSG  fromat
     $crs->ImportFromEPSG($epsg); }
  elsif ($epsg	=~ m/proj=/i) {			### Import Proj4 fromat
     $crs->ImportFromProj4($epsg);}
  elsif ($epsg	=~ m/GEOGCS/) {			### Import Wkt   fromat
     $crs->ImportFromWkt($epsg);  }
...
Usage:
	$app_name [-h] [-v] [-ot TYPE] [-a AGG_TYPE] [-so SCALE OFFSET] [-rm] [-rr NUMBER] [-p PATCH] [-trim] [-pd] [-mdt NAME:UNITS:FORMAT] [-sd YYYY-MM-DD] [-ed YYYY-MM-DD] [-mt MT_FILE] SRC_DATA_CODE DST_DATA_CODE
